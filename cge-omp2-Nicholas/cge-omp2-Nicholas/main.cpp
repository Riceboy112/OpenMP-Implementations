...
//#pragma omp barrier // Threads must wait until all threads reach this part.
        if (gmax == 0) {test = false; break;}
        
        //Swap rows if necessary
        if (gindmax != k)
        {
#pragma omp parallel for shared(a) firstprivate(n,k,gindmax) private(j,temp) schedule(static)
//#pragma omp parallel for schedule(static)
            for (j = k; j < n; j++)
            {
                temp = a[gindmax][j];
